...
void hmac_sha512(__generic uchar *key, int key_length_bytes, __generic uchar *message, int message_length_bytes, __generic uchar *output) {
  uchar ipad_key[128];
  uchar opad_key[128];
  for(int x=0;x<128;x++){
    ipad_key[x] = 0x36;
...
void serialized_public_key(extended_public_key_t *pub, __generic uchar *serialized_key) {
  secp256k1_ec_pubkey_serialize(serialized_key, 33, &pub->public_key.key, SECP256K1_EC_COMPRESSED);
}

void uncompressed_public_key(extended_public_key_t *pub, __generic uchar *serialized_key) {
  secp256k1_ec_pubkey_serialize(serialized_key, 65, &pub->public_key.key, SECP256K1_EC_UNCOMPRESSED);
}

void sha256d(__generic uchar *input, int input_len, __generic char * output) {
  sha256(input, input_len, output);
  sha256(output, 32, output);
}

void hash160(__generic uchar *input, int input_len, __generic char * output) {
  uchar sha256_result[32] = { 0 };
  sha256(input, input_len, sha256_result);
  ripemd160(sha256_result, 32, output);
}

void identifier_for_public_key(extended_public_key_t *pub, __generic uchar *identifier) {
  uchar serialized_key[33] = {0};
  serialized_public_key(pub, serialized_key);
  hash160(serialized_key, 33, identifier);
}

void fingerprint_for_public_key(extended_public_key_t *pub, __generic uchar *fingerprint) {
